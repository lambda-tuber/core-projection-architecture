...
module CPA.OntologicalWorld.ApplicationBase.ControlSpec (spec) where
...
import CPA.OntologicalWorld.ApplicationBase.Control (run)
...
sendMsg ctx msg = atomically $ writeTQueue (_ontologicalQueueGlobalContext ctx) msg
...
-- | semantic TQueue から受信（MagicalCalamity テスト用）
recvSemMsg :: GlobalContext -> IO Message
recvSemMsg ctx = atomically $ readTQueue (_semanticQueueGlobalContext ctx)

recvSemMsgTimeout :: GlobalContext -> IO (Maybe Message)
recvSemMsgTimeout ctx =
  atomically $ orElse
    (Just <$> readTQueue (_semanticQueueGlobalContext ctx))
    (return Nothing)
...
spec :: Spec
spec = do
  describe "CPA.OntologicalWorld.ApplicationBase.Control.run" $ do
    ...
    -- -----------------------------------------------------------------------
    -- MsgMagicalCalamity：転移プロトコル（ontological → semantic）
    -- -----------------------------------------------------------------------
    context "MsgMagicalCalamity" $ do
      ...
      around withWorldReady $ do
        ...
        context "MsgMagicalCalamity を送ったとき" $ do
          it "semantic の TQueue に MsgSetAvatar が届くこと" $ \ctx -> do
          ...
          it "転移後に MsgAttacked を送っても responseQueue に返らないこと（Avatar が消去済み）" $ \ctx -> do
            sendMsg ctx MsgMagicalCalamity
            threadDelay 20000
            -- semantic TQueue の MsgSetAvatar を読み捨て（テスト干渉回避）
            _ <- recvSemMsgTimeout ctx
            -- 転移後に Attacked を送る → vanishAvatar で Nothing → error → レスポンスなし
            sendMsg ctx (MsgAttacked 10)
            threadDelay 100000
