...
module CPA.OntologicalWorld.CoreModel.TH (
    funcTH_transit
...
--   instance IWorldState StartStateData where
...
-- RunState は引数ありコンストラクタのため以下のように扱う：
--   cur パターン: WorldStateW (RunState _)   （ワイルドカードで引数を無視）
--   nex 式      : WorldStateW initRunState   （initRunState を使う）
--
-- 生成されるコード例:
--   transit :: StateTransition -> WorldStateContext ()
--   transit StartToRun = lift get >>= \case
--     WorldStateW StartState   -> changeTo $ WorldStateW initRunState
--     WorldStateW x            -> throwError "invalid state transition. ..."
--   transit RunToStop = lift get >>= \case
--     WorldStateW (RunState _) -> changeTo $ WorldStateW StopState
--     WorldStateW x            -> throwError "invalid state transition. ..."
--
...
funcTH_transit = do
  ...
  where
    ...
    -- | "StartToRun" → (curPat, nexExpr) を返す
    --
    -- WorldState の GADT コンストラクタを reify して引数数を調べる。
    -- nameBase で比較することで修飾名の違いを吸収する。
    --   引数あり（RunState）→ cur: ConP 'WorldStateW [ConP curName [WildP]]
    --                        nex: VarE 'initRunState
    --   引数なし（StartState, StopState）→ cur: ConP 'WorldStateW [ConP curName []]
    --                                       nex: ConE nexName
    getStPat :: Name -> Q (Pat, Exp)
    getStPat n = do
      let modName = "CPA.OntologicalWorld.CoreModel.Type."
      ...
      let nexExpr = if nexArity > 0
            then VarE 'initRunState
            else ConE nexName
