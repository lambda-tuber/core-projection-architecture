...
module CPA.Request.CoreModel.Type where
...
-- | stdin から読み込んだ1行テキスト
-- work が T.Text を InputEventData に包んで EventW として yield する。
-- Run 状態のアクションが parseRequest を呼んで Request に変換する。
data InputEventData = InputEventData T.Text deriving (Show)
...
data Event r where
  ...
  InputEvent  :: InputEventData   -> Event InputEventData
