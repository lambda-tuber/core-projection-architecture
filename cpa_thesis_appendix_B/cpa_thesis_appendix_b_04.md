...
CPA のシステム状態は以下の4種に分類される。

| 種別 | 層 | 性質 | 形式化対象 |
|---|---|---|---|
| **Existential Info**（存在属性情報） | CoreModel | Avatar の HP・属性など「ただの値」。状態管理の対象ではない | 対象外（モデル定義のみ） |
| **Contextual State**（文脈的状態） | ProjectedContext | 射影計算中の一時的な記憶。SystemState から初期化され、フィードバックマッピングで返る | 対象外（ProjectedContextにおいて StateT として形式化済み） |
...
AppState は SystemState(ハードウェア・OS・ミドルウェア・ライブラリ・フレームワーク・アプリケーション内部の状態)における最上位に位置する。システムスタックのアプリケーション層より下位の状態は CPA の管理対象外である。AppState は **フレームワークのエントリポイントと ApplicationBase の境界** において明示的に定義・保持・遷移させる状態であり、CPA が責任を持つ唯一の「存在様式」の形式化対象である。
